...
2. Покажите пользователю `bankName` и `paymentTypeName`; в API создания PayIn-ордера или PayOut-ордера
...
4. Стройте поля формы только по `customerFields`. Для PayIn используйте `fields`
   ...
   PayIn-ордера. Набор может отличаться даже у методов с похожими названиями.
...
- `parallelGroupOrdersEnabled`: метод допускает параллельные ордера в группе;
