...
5. Если запрос не завершился стандартным ответом API с явным отказом, считайте
   ордер созданным и проверьте его через [`GET /shop/payout-orders/external/{id}`](/doc/api/payout/03-read-and-cancel/#get-shoppayout-ordersexternalid).
...
`5xx`, — ордер следует считать созданным. Перед повторной попыткой проверьте его
статус через [`GET /shop/payout-orders/external/{id}`](/doc/api/payout/03-read-and-cancel/#get-shoppayout-ordersexternalid),
чтобы избежать дублирования заявок. Не обходите проверку новым `externalOrderId`.
...
| Нет стандартного ответа API с явным отказом | Считайте выплату созданной и проверьте её через `GET /shop/payout-orders/external/{id}`, чтобы избежать дублирования заявок |
