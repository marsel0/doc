...
| --- | --- | --- | --- |
| PayIn Redirect | Площадка возвращает `integration.link`, на который нужно перевести клиента | Создание ордера, переход по ссылке и получение статуса. Метод и реквизиты клиент выбирает на странице площадки | [Redirect](/doc/v2/red/) |
...
| PayIn H2H по шагам — **сложный, не рекомендуется** | Ордер обязательно создать до выбора способа оплаты | Получение методов, выбор через `PATCH`, запуск поиска реквизитов, подтверждение перевода и получение статуса | [H2H: по шагам](/doc/v2/h2h-step/) |
| PayOut H2H | Магазин выплачивает деньги клиенту по его реквизитам | Сбор реквизитов, проверку баланса, создание выплаты и получение статуса | [PayOut](/doc/v2/payout/) |
...
| Получить статус | Уведомление на `callbackUrl`; для проверки — [`GET /shop/orders/{id}`](/doc/api/payin/03-read/#по-внутреннему-id) | [Чтение ордера](/doc/api/payin/03-read/), [уведомления](/doc/v2/callback-signature/) |
...
6. Проверьте уведомления для успешного и отменённого ордера, включая повторную доставку.
...
Если запрос создания завершился
