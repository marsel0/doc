...
* **Ключ подписи запросов** — нужен для проверки подлинности получаемых вебхуков.
...
API-ключ иили ключ для работы с балансами передаются в заголовке запросов:
...
Для дополнительной защиты можно указать IP-адреса, с которых разрешены запросы к магазину (в настройках магазина в личном кабинете). Запросы с других IP, даже с верным ключом, будут отклоняться.
